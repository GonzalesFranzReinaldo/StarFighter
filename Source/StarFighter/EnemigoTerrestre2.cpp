...
void AEnemigoTerrestre2::BeginPlay()
{
	...
	GetWorldTimerManager().SetTimer(MemberTimerHandle, this, &AEnemigoTerrestre2::FireEnemigo, 01.0f, true, 3.0f);
}
...
void AEnemigoTerrestre2::FireEnemigo()
{
	bCanFire = true;
...
void AEnemigoTerrestre2::FireShotEnemigo(FVector FireDirectionEnemigo)
{
	if (bCanFire == true)
	{
		if (FireDirectionEnemigo.SizeSquared() > 0.0f)
		{
			...
			bCanFire = false;
			World->GetTimerManager().SetTimer(TimerHandle_ShotTimerExpired, this, &AEnemigoTerrestre2::ShotTimerExpired, FireRate);
...
void AEnemigoTerrestre2::ShotTimerExpired()
{
	bCanFire = true;
}
